...
import { loginUser, registerUser, logoutUser, checkAuthStatusThunk } from './authThunks';
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(loginUser.rejected, (state, action) => {
                state.status = 'failed';
                state.user = null;
                state.error = (action.payload as ErrorResponse)?.message || action.error.message || 'Login Failed';
            })
            ...
            .addCase(registerUser.rejected, (state, action) => {
                state.status = 'failed';
                state.user = null;
                state.error = (action.payload as ErrorResponse)?.message || action.error.message || 'Signup Failed';
            })
